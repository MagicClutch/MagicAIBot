//! Conservative vertical-route policy used by the pathfinder integration.
//! Execution remains event-driven: a route action is not complete until the
//! executor observes its world-state postcondition.
...
pub const PLACE_COST: u32 = 12;
pub const PILLAR_COST: u32 = 20;

/// Pick only a bounded, safe vertical primitive. The executor must still
/// select an allowed block/tool and confirm the resulting world update.
...
pub fn action_cost(action: VerticalAction) -> u32 {
    match action {
        ...
        VerticalAction::PlaceBelow => PLACE_COST,
...
/// [`ScaffoldPolicy::denied`], not a replacement for it.
...
/// Which blocks are acceptable scaffolding and how many are required before
/// a route is allowed to rely on them.
#[derive(Clone, Debug, Default)]
pub struct ScaffoldPolicy {
    /// Preference order: the first entry the bot holds enough of wins.
    pub allowed: Vec<String>,
    pub denied: Vec<String>,
    pub minimum_held: u32,
}

/// Chooses the first configured scaffold block (in preference order) that
/// `counts` reports at least `policy.minimum_held` of, skipping anything on
...
/// `counts` is a plain item-id -> held-count map so this stays independent
/// of any particular inventory-snapshot type.
pub fn select_scaffold_block(
    ...
    policy
        .allowed
        .iter()
        .filter(|id| !policy.denied.contains(id))
        .filter(|id| !is_hard_denied(id))
        .find(|id| counts.get(id.as_str()).copied().unwrap_or(0) >= policy.minimum_held)
        .cloned()
}
...
pub fn dominant_step_toward(from: BlockPos, to: BlockPos) -> (i32, i32) {
...
mod tests {
    ...
    #[test]
    fn descent_refuses_lava_and_falls() {
        let mut s = safety();
        s.lava_below = true;
        assert_eq!(
    ...
    fn scaffold_policy() -> ScaffoldPolicy {
        ScaffoldPolicy {
            allowed: [
                "minecraft:dirt",
    ...
    fn counts(items: &[(&str, u32)]) -> std::collections::HashMap<String, u32> {
        items.iter().map(|(k, v)| (k.to_string(), *v)).collect()
    }

    #[test]
    fn scaffold_selection_follows_preference_order() {
        let counts = counts(&[("minecraft:cobblestone", 10), ("minecraft:dirt", 20)]);
        assert_eq!(
            select_scaffold_block(&scaffold_policy(), &counts).as_deref(),
            Some("minecraft:dirt")
        );
    }
    #[test]
    fn scaffold_selection_skips_below_minimum_count() {
        let counts = counts(&[("minecraft:dirt", 1), ("minecraft:stone", 10)]);
        assert_eq!(
            select_scaffold_block(&scaffold_policy(), &counts).as_deref(),
    ...
    fn scaffold_selection_never_uses_denied_or_valuable_blocks() {
        let mut policy = scaffold_policy();
        policy.allowed.push("minecraft:diamond_block".into());
        policy.allowed.push("minecraft:iron_pickaxe".into());
        let counts = counts(&[
            ("minecraft:diamond_block", 64),
            ("minecraft:iron_pickaxe", 1),
        ]);
        assert_eq!(select_scaffold_block(&policy, &counts), None);
    }
    #[test]
    fn scaffold_selection_returns_none_when_nothing_suitable_is_held() {
        assert_eq!(
            select_scaffold_block(&scaffold_policy(), &counts(&[])),
            None
        );
    }
