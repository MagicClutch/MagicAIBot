...
use azalea_client::{
    ClientMovementState, StartSprintEvent, StartWalkEvent,
...
use crate::{
    ...
    pathfinder::{
        ...
        moves::{BlockedOnMissingScaffold, ExecuteCtx, IsReachedCtx, PillarClimbBaseline, Placing},
        player_pos_to_block_pos,
        policy::refresh_pathfinding_policy,
    },
};

/// Fully stops the entity's movement input -- walking *and* sneaking --
/// whenever a path completes or gets cleared out from under it.
///
/// A build move (bridge/pillar/staircase) can leave `trying_to_crouch` set
/// on its very last active tick (e.g. a bridge move sneaking to place the
/// final block right as the goal is reached), and nothing else naturally
/// resets it: sneaking is normally cleared once per tick by
/// `ExecuteCtx::on_tick_start`, but that only runs for entities
/// `tick_execute_path` still visits, which requires a live `ExecutingPath`
/// -- exactly what's being removed at every call site of this function.
/// Without this, the bot would stay sneaking indefinitely after finishing
/// (or having its) a route.
fn stop_moving_and_sneaking(
    commands: &mut Commands,
    walk_events: &mut MessageWriter<StartWalkEvent>,
    entity: Entity,
) {
    ...
    commands
        ...
        .queue(move |mut entity: EntityWorldMut<'_>| {
            if let Some(mut movement_state) = entity.get_mut::<ClientMovementState>() {
                movement_state.trying_to_crouch = false;
            }
        });
}
...
pub fn tick_execute_path(
    ...
    mut place_item_events: MessageWriter<StartUseItemEvent>,
) {
    for (
        entity,
        mut executing_path,
    ...
    ) in &mut query
    {
        ...
        if let Some(edge) = executing_path.path.front() {
            let mut ctx = ExecuteCtx {
                ...
                custom_state: custom_state.map_or_else(Default::default, |s| s.0.clone()),
                pillar_climb_baseline: pillar_climb_baseline.copied(),
                ...
                place_item_events: &mut place_item_events,
            };
            ctx.on_tick_start();
            trace!(
        ...
        'skip: loop {
                ...
                .rev()
            {
                    ...
                    && executing_path.is_empty_queued_path()
                {
                    ...
                    // Genuinely at rest (no horizontal velocity at all) means nothing is going
                    // to carry us further regardless of where in the block we're standing, so
                    // the offset-from-center prediction below doesn't apply. Without this,
                    // techniques that cross onto the final block near its edge rather than
                    // walking to dead-center (e.g. fast bridging) could satisfy every other
                    // condition here yet sit just past the 0.2 offset forever, since a fixed
                    // off-center resting spot never becomes "more centered" by waiting -- the
                    // path would never formally complete even though the bot is safely
                    // stopped.
                    let at_rest =
                        physics.velocity.x.abs() < 0.005 && physics.velocity.z.abs() < 0.005;
                    ...
                    (physics.on_ground() || physics.is_in_water())
                        ...
                        && (at_rest || (x_predicted_offset < 0.2 && z_predicted_offset < 0.2))
                } else {
                    true
                };
                ...
                if (movement.data.is_reached)(is_reached_ctx) && extra_check {
                    ...
                    if let Some(new_path) = executing_path.queued_path.take() {
                        ...
                        if executing_path.path.is_empty() {
                            ...
                            stop_moving_and_sneaking(&mut commands, &mut walk_events, entity);
...
pub fn timeout_movement(
    mut query: Query<(
        ...
        Option<&Mining>,
        Option<&Placing>,
        Option<&BlockedOnMissingScaffold>,
        &WorldName,
        &Inventory,
        Option<&CustomPathfinderState>,
    ...
    ) in &mut query
    {
        ...
        // don't timeout if we're placing a block: placement has no client-side
        // prediction (unlike mining), so confirmation can take several ticks. The
        // tolerance is tighter than mining's since placement requires standing
        // still, close to the reference block, rather than just being nearby.
        if let Some(placing) = placing
            && placing.pos.distance_squared_to(position.into()) < 3_i32.pow(2)
        {
            executing_path.ticks_since_last_node_reached = 0;
            continue;
        }
        ...
        {
            if blocked_on_missing_scaffold.is_some() {
                // A build move (bridge/pillar/staircase) has been stalled
                // this whole timeout waiting on scaffold material that
                // never showed up. Patching would just search for a new
                // path with the exact same policy and the exact same empty
                // inventory, which can't do any better -- so this cancels
                // the goal outright instead of retrying forever.
                warn!("pathfinder cannot continue: no scaffold material available, cancelling");
                stop_moving_and_sneaking(&mut commands, &mut walk_events, entity);
                commands.entity(entity).remove::<ExecutingPath>();
                commands.entity(entity).remove::<BlockedOnMissingScaffold>();
                pathfinder.goal = None;
                pathfinder.opts = None;
                continue;
            }
...
fn patch_path_from_timeout(
    ...
    if let Some(custom_state) = custom_state {
        refresh_pathfinding_policy(custom_state, &inventory.inventory_menu);
    }
