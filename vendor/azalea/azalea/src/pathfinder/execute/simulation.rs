...
use crate::{
    ...
    pathfinder::{
        ...
        moves::{ExecuteCtx, IsReachedCtx, PillarClimbBaseline},
        ...
        for _ in 1..=30 {
...

            {
                let mut system_state = SystemState::<(
                    ...
                    MessageWriter<StartUseItemEvent>,
                ...
                let (
                    ...
                    mut place_item_events,
                ) = system_state.get_mut(sim.app.world_mut()).unwrap();

                let (position, physics, mining, inventory, custom_state, pillar_climb_baseline) =
                ...
                (next_node.movement.data.execute)(ExecuteCtx {
                    ...
                    can_mine: false,
                    // placement has no server round-trip to observe within this
                    // in-memory lookahead, so never dispatch it here either
                    can_place: false,
                    world: sim.world.clone(),
                    menu: inventory.inventory_menu.clone(),
                    custom_state: custom_state.map_or_else(Default::default, |s| s.0.clone()),
