...
pub struct Pathfinder {
    ...
    /// How many searches in a row have come back with no usable path at all
    /// (as opposed to timing out from progress elsewhere, like patching
    /// around an obstruction while a path is still executing). Reset to 0
    /// every time a search actually finds somewhere to go.
    ///
    /// `retry_on_no_path` exists for goals that are temporarily unreachable
    /// (e.g. a transient obstruction) and expected to resolve on their own,
    /// but without a cap this also retries goals that are *permanently*
    /// unreachable -- e.g. a bridge that needs more scaffold material than
    /// the bot has and will ever have -- every tick, forever, each retry
    /// re-running a full (up to million-node) search. See
    /// `EMPTY_PATH_RETRY_LIMIT`.
    pub consecutive_empty_paths: u32,
}
...
pub fn goto_listener(
    ...
    for event in events.read() {
        ...
        if let Some(custom_state) = custom_state {
            policy::refresh_pathfinding_policy(custom_state, &inventory.inventory_menu);
        }
...
/// See [`Pathfinder::consecutive_empty_paths`].
const EMPTY_PATH_RETRY_LIMIT: u32 = 5;
...
pub fn path_found_listener(
    ...
    for event in events.read() {
        ...
        if let Some(found_path) = &event.path {
            if let Some(mut executing_path) = executing_path {
                pathfinder.consecutive_empty_paths = 0;
                ...
                if !pathfinder.opts.as_ref().is_some_and(|o| o.retry_on_no_path) {
                    ...
                    pathfinder.consecutive_empty_paths += 1;
                    if pathfinder.consecutive_empty_paths >= EMPTY_PATH_RETRY_LIMIT {
                        // `retry_on_no_path` is meant for goals that are only
                        // *temporarily* unreachable (e.g. a transient
                        // obstruction) and expected to resolve on their own --
                        // not goals that can never be reached at all, like a
                        // bridge that needs more scaffold material than the
                        // bot has and will ever have. Without this cap that
                        // case retries a full search every tick, forever,
                        // instead of ever giving up.
                        warn!(
                            "pathfinder found no usable path {} times in a row, giving up and cancelling the goal",
                            pathfinder.consecutive_empty_paths
                        );
                        pathfinder.goal = None;
                        pathfinder.consecutive_empty_paths = 0;
                    }
