...
impl Plugin for InventoryPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            GameTick,
            (
                // Ordered after both mining's and placement's queued-action
                // handlers so the hotbar-slot packet (ServerboundSetCarriedItem)
                // is guaranteed to reach the server before a same-tick mine or
                // place packet that depends on the new slot being selected.
                // Without the placement edge, the scheduler treats the two as
                // ambiguous and either order is possible within one tick.
