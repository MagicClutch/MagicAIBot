...
pub struct StartUseItemEvent {
    ...
    /// See [`StartUseItemQueued::force_direction`].
...
pub struct StartUseItemQueued {
    ...
    pub force_block: Option<BlockPos>,
    /// Which face of `force_block` to report as clicked when the real hit
    /// result doesn't already match it, i.e. which side the placed block
    /// ends up on. Ignored unless `force_block` is `Some`. Defaults to
    /// [`Direction::Up`] (place on top of the block) when `None`, matching
    /// this field's pre-existing behavior.
    pub force_direction: Option<Direction>,
}
...
pub fn handle_start_use_item_queued(
        ...
        query
    {
        ...
        if let Some(force_block) = start_use_item.force_block {
            // The real raycast matching `force_block`'s position isn't
            // enough on its own -- if a specific `force_direction` was
            // requested, the real hit's face has to match too, or the
            // caller's explicitly chosen face gets silently discarded in
            // favor of whatever face the real look angle happened to hit
            // (e.g. looking down-and-behind at a reference block to place
            // against its side face, but genuinely being close enough to
            // it that the raycast reports the top face instead). That
            // silently placed against the wrong face, which the server can
            // reject outright -- looking like the placement never
            // happened at all.
            let hit_result_matches = if let HitResult::Block(block_hit_result) = &hit_result {
                block_hit_result.block_pos == force_block
                    && start_use_item
                        .force_direction
                        .is_none_or(|force_direction| block_hit_result.direction == force_direction)
            } else {
                false
            };

            if !hit_result_matches {
                // we're not looking at the block (or the right face of
                // it), so make up some numbers
                hit_result = HitResult::Block(BlockHitResult {
                    location: force_block.center(),
                    direction: start_use_item.force_direction.unwrap_or(Direction::Up),
