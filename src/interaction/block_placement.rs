...
impl PlacementMachine {
    ...
    pub fn advance(&mut self, observation: PlacementObservation) {
        if observation.cancelled {
            self.fail(PlacementFailure::Cancelled);
            return;
        }
        if !observation.connected {
            self.fail(PlacementFailure::Disconnected);
            return;
        }
        if !observation.alive {
            self.fail(PlacementFailure::Died);
            return;
        }
        if observation.total_expired {
            self.fail(PlacementFailure::TimedOut);
            return;
        }
        self.phase = match self.phase {
            ...
            PlacementPhase::SelectingHit if observation.hit_selected => {
                if observation.in_range {
                    PlacementPhase::Rotating
                } else {
                    PlacementPhase::Navigating
                }
            }
            PlacementPhase::Navigating if observation.navigation_finished => {
                PlacementPhase::Rotating
            }
            PlacementPhase::Rotating if observation.rotation_finished => {
                PlacementPhase::PreparingHotbar
            }
            PlacementPhase::PreparingHotbar if observation.hotbar_confirmed => {
                PlacementPhase::Dispatching
            }
            ...
            PlacementPhase::Confirming
                if observation.world_confirmed && observation.inventory_confirmed =>
            {
                self.result = Some(BlockPlacementResult::Placed {
                    item: String::new(),
            ...
            PlacementPhase::Confirming if observation.attempt_expired => {
                if self.attempts > self.maximum_retries {
                    self.result = Some(BlockPlacementResult::Failed(
                        PlacementFailure::ServerRejected,
                    ));
                    PlacementPhase::Finished
                } else {
                    PlacementPhase::Retrying
                }
            }
...
mod tests {
    use super::*;
    fn observed() -> PlacementObservation {
    ...
    fn mocked_machine_visits_every_phase_and_requires_dual_confirmation() {
        ...
        o.valid = true;
        machine.advance(o);
        assert_eq!(machine.phase, PlacementPhase::SelectingHit);
        let mut o = observed();
        o.hit_selected = true;
        machine.advance(o);
        assert_eq!(machine.phase, PlacementPhase::Navigating);
        let mut o = observed();
        o.navigation_finished = true;
        machine.advance(o);
        assert_eq!(machine.phase, PlacementPhase::Rotating);
        let mut o = observed();
        o.rotation_finished = true;
        machine.advance(o);
        assert_eq!(machine.phase, PlacementPhase::PreparingHotbar);
        let mut o = observed();
        o.hotbar_confirmed = true;
        machine.advance(o);
        assert_eq!(machine.phase, PlacementPhase::Dispatching);
        let mut o = observed();
        o.interaction_sent = true;
        machine.advance(o);
        assert_eq!(machine.phase, PlacementPhase::Confirming);
        let mut o = observed();
        o.world_confirmed = true;
        machine.advance(o);
        assert_eq!(machine.phase, PlacementPhase::Confirming);
        let mut o = observed();
        o.world_confirmed = true;
        o.inventory_confirmed = true;
        machine.advance(o);
        assert_eq!(machine.phase, PlacementPhase::Finished);
    }
    #[test]
    fn mocked_retry_is_bounded() {
        let mut machine = PlacementMachine::new(0);
        machine.phase = PlacementPhase::Confirming;
        machine.attempts = 1;
        let mut o = observed();
        o.attempt_expired = true;
        machine.advance(o);
        assert_eq!(
            machine.result,
            Some(BlockPlacementResult::Failed(
                PlacementFailure::ServerRejected
            ))
        );
    }
    ...
    fn terminal_guards_cover_cancellation_death_disconnect_and_timeout() {
        for (observation, expected) in [
            ...
            (
                ...
                PlacementFailure::TimedOut,
            ),
        ] {
            let mut m = PlacementMachine::new(1);
            m.advance(observation);
            assert_eq!(m.result, Some(BlockPlacementResult::Failed(expected)));
        }
    }
    #[test]
    fn failures_are_typed_for_each_adapter_boundary() {
        let failures = [
            PlacementFailure::WrongDimension,
            PlacementFailure::Unloaded,
            PlacementFailure::OutsideWorld,
            PlacementFailure::Forbidden,
            PlacementFailure::TargetOccupied,
            PlacementFailure::TargetNotReplaceable,
            PlacementFailure::TargetObstructed,
            PlacementFailure::NoSupportFace,
            PlacementFailure::SupportChanged,
            PlacementFailure::ItemMissing,
            PlacementFailure::ItemInaccessible,
            PlacementFailure::InventoryFull,
            PlacementFailure::InventoryStale,
            PlacementFailure::OutOfRange,
            PlacementFailure::Unreachable,
            PlacementFailure::PathFailed,
            PlacementFailure::RotationFailed,
            PlacementFailure::ResultNotConfirmed,
        ];
        for failure in failures {
            let mut m = PlacementMachine::new(1);
            m.fail(failure.clone());
            assert_eq!(m.result, Some(BlockPlacementResult::Failed(failure)));
        }
    }
}
